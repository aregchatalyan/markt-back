...
export const validationMiddleware = (checks) => async (req, res, next) => {
  ...
  for (const { path, where = 'body', ...check } of checks) {
    ...
    if (check.length?.min) chain.trim().isLength({ min: check.length.min }).withMessage(messages('length.min', path, income, check.length.min));

    if (check.length?.max) chain.trim().isLength({ max: check.length.max }).withMessage(messages('length.max', path, income, check.length.max));

    if (check.email) chain.trim().toLowerCase().isEmail({ host_whitelist: [ 'gmail.com' ] }).withMessage(messages('email', path, income));

    if (check.phone) chain.trim().isMobilePhone('any', { strictMode: true }).withMessage(messages('phone', path, income));
